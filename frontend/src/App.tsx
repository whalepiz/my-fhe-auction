...
import {
  CHAIN_ID,
  AUCTIONS as ENV_AUCTIONS,
  AUCTION_META,
  RPCS,
} from "./config";
import {
  encryptUint32WithProof,
  waitForPublicKey,
} from "./lib/fhe";
...
export default function App() {
  ...
  const [listStatus, setListStatus] = useState<
    Record<string, AuctionStatus | null>
  >({});
  ...
  /* Actions */
  async function submitBid(ev: React.FormEvent<HTMLFormElement>) {
    ev.preventDefault();
    ...
    if (Number(detail?.endTime || 0) <= nowSec())
      return setToast("Phiên đã kết thúc.");
    ...
    try {
      ...
      const link = txHash
        ...
        : "";
      const hint =
        /execution reverted|InvalidInput|Cipher|Proof|PublicKey|Invalid|input|kms/i.test(
          base
        )
          ? " (Mẹo: đợi 20–60s cho FHE key/proof sẵn sàng rồi thử lại.)"
          : "";
      setToast("Bid thất bại: " + base + hint + link);
    } finally {
      setBusy(null);
    }
  }
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    {st && (
                      <div>
                        {!st.settled && !endedCard && (
                          <Badge color="green">Ongoing</Badge>
                        )}
                        {!st.settled && endedCard && (
                          <Badge color="orange">Ended</Badge>
                        )}
              ...
              >
                <label style={{ display: "grid", gap: 6 }}>
                  ...
                  <input
                    ...
                    disabled={
                      !detail || Number(detail?.endTime || 0) <= nowSec() || !!busy
                    }
                    style={{
                      width: "100%",
                      padding: 10,
                ...
                <button
                  type="submit"
                  disabled={
                    !!busy || !detail || Number(detail?.endTime || 0) <= nowSec()
                  }
                  style={{
                    padding: "10px 16px",
                    borderRadius: 8,
