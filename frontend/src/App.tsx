...
// Thử lần lượt BrowserProvider(sepolia) -> nhiều JsonRpcProvider cho đến khi OK
...
// Đọc trạng thái an toàn: nếu decode lỗi ở 1 RPC, thử RPC khác;
// nếu tất cả đều thất bại → trả null (đánh dấu “incompatible”)
  ...
  async function settleAndReveal() {
    ...
    try {
      ...
      const frag: Fragment | null = (() => {
        try {
          return iface.getFunction("settle");
        } catch {
          return null;
        }
      })();
    ...
    <div style={{ maxWidth: 980, margin: "24px auto", fontFamily: "Inter, system-ui" }}>
              ...
              >
                ...
                <div style={{ fontSize: 12, opacity: 0.8 }}>
                  ...
                  {incompatible
                    ? "-"
                    : st
                    ? Number(st.endTime) > Date.now() / 1000
                      ? "ongoing"
                      : "ended"
                    : "-"}{" "}
                  · {st ? fmtTs(st.endTime) : "-"}
        ...
        <div style={{ borderTop: "1px solid #eee", marginTop: 16, paddingTop: 16 }}>
          ...
          >
            ...
            <div>
              ...
              {detail
                ? Number(detail.endTime) > Date.now() / 1000
                  ? "ongoing"
                  : "ended"
                : "-"}
            </div>
          ...
          <form
            onSubmit={submitBid}
            style={{ display: "grid", gap: 12, maxWidth: 560, marginTop: 16 }}
          >
            <label>
              Your bid (uint32)
              <input
      ...
      {!addresses.length && (
        <div style={{ marginTop: 16, color: "#b91c1c" }}>
          Chưa cấu hình địa chỉ. Hãy set env <code>VITE_AUCTIONS</code> hoặc{" "}
          <code>VITE_AUCTION_ADDRESS</code>.
        </div>
      )}
    </div>
  );
